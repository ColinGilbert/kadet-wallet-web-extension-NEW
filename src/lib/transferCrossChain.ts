...
import { listen, pollCreateSpv, pollStatus, submit } from './utils/client';
import { inspect } from './utils/fp-helper';
...
export function transferCrossChain(
  ...
  const senderPubkey = store.getState().walletState.publicKey;
  const senderSecret = store.getState().walletState.secretKey;

  const from: IAccount = {
    account: 'k:' + senderPubkey,
    chainId: store.getState().walletState.chainId as ChainwebChainId,
    publicKey: senderPubkey,
    // use keyset guard
    guard: senderPubkey,
  };
